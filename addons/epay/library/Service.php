...
class Service
{
    ...
    public static function submitOrder($amount, $orderid = null, $type = null, $title = null, $notifyurl = null, $returnurl = null, $method = null, $openid = '')
    {
        ...
        $request = request();
        $notifyurl = $params['notifyurl'] ?? $request->root(true) . '/addons/epay/index/' . $type . 'notify';
        $returnurl = $params['returnurl'] ?? $request->root(true) . '/addons/epay/index/' . $type . 'return/out_trade_no/' . $orderid;
        $html = '';
     ...
     * @return mixed|string
     */
    public static function getOpenid($apptype = 'wechat')
    {
        $config = self::getConfig($apptype);
        $openid = '';
        $auth = Auth::instance();
        if ($auth->isLogin()) {
            $third = get_addon_info('third');
            if ($third && $third['state']) {
                $thirdInfo = Third::where('user_id', $auth->id)->where('platform', 'wechat')->find();
