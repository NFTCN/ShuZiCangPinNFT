...
class NftService
{
     ...
     */
    public function createTokenId(string $no, string $author, string $nft, string $salt): NftService
    {
        //对图像的hash 进行存储 防止oss 过度读写
        if(Cache::has(cdnurl($nft,true))){
            $nft = Cache::get(cdnurl($nft,true));
        }else{
            $nft = hash_file($this->hashMethod, cdnurl($nft,true));
            Cache::set(cdnurl($nft,true),$nft);
        }
        if(!empty($salt)){
            $this->tokenId = ($this->hashMethod)($no . $author . $nft.$salt);

        }else{
            $this->tokenId = ($this->hashMethod)($no . $author . $nft);
        }
        return $this;
    ...
    public function checkHash($name = ''): bool
    {
        ...
        foreach ($list as $item) {
            if (empty($hash)) {
                ...
                $hash = ($this->hashMethod)($hash . $item->owner);
            }
